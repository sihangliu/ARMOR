...
values_8, base_8 = np.histogram(y_8, bins=500)
values_rand, base_rand = np.histogram(y_rand, bins=500)
...
plt.hold(True)
plt.xlabel('Absolute Error', fontsize=20)
...
l1 = plt.plot(base_8[:-1], cumulative_8,'-', color = 'b', label=xname, markersize=5, lw=4)
...
l4 = plt.plot([1, 1], [min_y, 1], 'k--', lw=3, label="Worst-case Bound")
...
art = []
art.append(lgd)
plt.savefig('sobel_error_cdf.png', additional_artists=art, bbox_inches='tight', dpi=400)
#plt.savefig('sobel_error_cdf.pdf', additional_artists=art, bbox_inches='tight')
